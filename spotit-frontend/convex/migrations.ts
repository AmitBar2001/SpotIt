...
export const getSongsToMigrate = internalQuery({
  ...
  handler: async (ctx) => {
    ...
    return songs.filter((song) => {
        ...
        return typeof metadata.album === "string" || metadata.album === undefined || !metadata.album.images;
    });
  },
});
...
export const enrichMetadataAction = internalAction({
  ...
  handler: async (ctx) => {
    ...
    for (const song of songs) {
      ...
      try {
        const response = await fetch(`${backendUrl}/search-track/?query=${encodeURIComponent(query)}`, {
             headers: {
        ...
        if (response.ok) {
            ...
            const mergedMetadata = {
                ...newMetadata,
                youtube_views: currentMetadata.youtube_views || newMetadata.youtube_views,
